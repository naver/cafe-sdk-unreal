...
#define kThirdPartyMainWindowWidth ((int)UIScreen.mainScreen.applicationFrame.size.width)
#define kThirdPartyMainWindowHeight ((int)UIScreen.mainScreen.applicationFrame.size.height)
...
- (void)viewWillAppear:(BOOL)animated   {
    if(_linkUrl != nil) {
        NSString *escapedString = [_linkUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (NSString *)parameterValueWithUrl:(NSString *)url paramName:(NSString *)name
{
    ...
    for ( NSString *param in separtedParam )
    {
        ...
        {
            return [[param substringFromIndex:[name length]+1] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];	//ex) @"url="
        }
    }
    return nil;
}
